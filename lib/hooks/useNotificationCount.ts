...
export function useNotificationCount(): number | null {
  ...
  useEffect(() => {
    const wallet = typeof window !== 'undefined' ? localStorage.getItem('wallet_address') : null;
    ...
    const loadCount = async () => {
      try {
        ...
        const res = await fetch(
          `/api/notifications?wallet=${encodeURIComponent(normalizedWallet)}&archived=false`
        );
