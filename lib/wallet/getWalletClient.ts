  ...
  if (walletType === 'walletconnect') {
    ...
    if (!isWalletConnectEnabled) {
      ...
      const error = new Error(
        'WalletConnect is temporarily unavailable. Please use MetaMask or reconnect via /auth to use a different wallet.'
      );
 ...
 */
export function setWalletType(
  walletAddress: `0x${string}`,
  type: 'metamask' | 'passkey' | 'walletconnect'
) {
  if (typeof window !== 'undefined') {
...
export function getWalletType(
  walletAddress: `0x${string}`
): 'metamask' | 'passkey' | 'walletconnect' | null {
  if (typeof window === 'undefined') {
    return null;
  }
