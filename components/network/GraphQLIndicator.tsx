...
export function GraphQLIndicator() {
  ...
  return (
    <div
      className="absolute right-4 top-4 z-30 rounded-lg border-2 bg-gray-900/90 px-4 py-2.5 text-sm shadow-lg backdrop-blur-md"
      style={{
        borderColor: isUsingGraphQL ? 'rgba(16, 185, 129, 0.5)' : 'rgba(59, 130, 246, 0.5)',
      }}
    >
      <div className="flex items-center gap-2.5">
        <div
          className={`h-3 w-3 rounded-full ${isUsingGraphQL ? 'bg-emerald-400 shadow-[0_0_8px_rgba(16,185,129,0.6)]' : 'bg-blue-400 shadow-[0_0_8px_rgba(59,130,246,0.6)]'}`}
        />
      ...
      {isUsingGraphQL && (
        <div className="mt-1.5 font-mono text-[10px] text-emerald-400/80">→ /api/graphql</div>
      )}
      {!isUsingGraphQL && (
        <div className="mt-1.5 font-mono text-[10px] text-blue-400/80">→ /api/network/graph</div>
      )}
    </div>
  );
}
