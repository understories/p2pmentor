      ...
      {hasFilter && (
        <div className="mb-3 flex flex-wrap gap-2">
          {filterWords.map((word, index) => (
            <div
              key={index}
              className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-sm font-medium transition-all duration-200"
              style={{
                backgroundColor:
      ...
      {arkivBuilderMode ? (
        ...
        >
          <input
            ...
            className="w-full rounded-xl border px-4 py-3 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-emerald-500/50"
            style={{
              backgroundColor:
                theme === 'dark' ? 'rgba(31, 41, 55, 0.6)' : 'rgba(255, 255, 255, 0.9)',
