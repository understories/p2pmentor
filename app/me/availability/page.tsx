...
export default function AvailabilityPage() {
  ...
  const loadData = async (wallet: string) => {
    try {
      ...
      const [profileData, availabilitiesRes] = await Promise.all([
        ...
        fetch(`/api/availability?wallet=${encodeURIComponent(wallet)}`)
          .then((r) => r.json())
          .catch(() => ({ ok: false, availabilities: [] })),
      ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.ok) {
        if (data.pending) {
          setSuccess(
            'Availability submitted! Transaction is being processed. Please refresh in a moment.'
          );
          ...
          setSuccess(
            editingKey ? 'Availability updated successfully!' : 'Availability created successfully!'
          );
  ...
  const handleEdit = (availability: Availability) => {
    ...
    if (isStructured) {
      ...
      if (structured) {
        ...
        setError(
          'Cannot edit legacy availability format. Please create a new availability block using the weekly schedule.'
        );
        return;
      }
    } else {
    ...
    // Scroll to form
    window.scrollTo({
      top: document.getElementById('availability-form')?.offsetTop || 0,
      behavior: 'smooth',
    });
  };
  ...
  const handleDelete = async (availabilityKey: string) => {
    ...
    if (
      !confirm(
        'Are you sure you want to delete this availability block? This action cannot be undone.'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (data.ok) {
        if (data.pending) {
          setSuccess(
            'Deletion submitted! Transaction is being processed. Please refresh in a moment.'
          );
          setTimeout(() => loadData(walletAddress), 2000);
        } else {
    ...
    <div className="min-h-screen p-4 text-gray-900 dark:text-gray-100">
      <div className="mx-auto max-w-2xl">
        ...
        {availabilities.length > 0 && (
          <div className="mb-8">
            <h2 className="mb-4 text-xl font-semibold">Your Availability Blocks</h2>
            <div className="space-y-3">
              {availabilities.map((availability) => {
                // Check if structured format and format accordingly
                const isStructured = isStructuredAvailability(availability.timeBlocks);
                const structuredAvail = isStructured
                  ? deserializeWeeklyAvailability(availability.timeBlocks)
                  : null;
                    ...
                    <div className="mb-2 flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-blue-700 dark:text-blue-300">
                          ...
                          {isStructured && (
                            <span className="ml-2 rounded bg-blue-200 px-2 py-0.5 text-xs dark:bg-blue-800">
                              Structured
                            </span>
                          )}
                        </p>
                        ...
                        {arkivBuilderMode ? (
                          ...
                          >
                            <button
                              ...
                              className="ml-4 rounded px-3 py-1 text-sm text-blue-600 transition-colors hover:bg-blue-50 disabled:opacity-50 dark:text-blue-400 dark:hover:bg-blue-900/20"
                            >
        ...
        {profile?.availabilityWindow && availabilities.length === 0 && (
          <div className="mb-6 rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
            <p className="mb-1 text-sm font-medium text-yellow-900 dark:text-yellow-200">
        ...
        <div id="availability-form" className="mb-8">
          ...
          {editingKey && (
            <div className="mb-4 rounded-lg border border-yellow-200 bg-yellow-50 p-3 dark:border-yellow-800 dark:bg-yellow-900/20">
              <p className="text-sm text-yellow-800 dark:text-yellow-200">
                <strong>Editing mode:</strong> Modifying an availability block will create a new
                entity. The original will remain on Arkiv until it expires.
              </p>
            </div>
          )}
            ...
            <div className="flex gap-3">
              ...
              {editingKey ? (
                <button
                  ...
                  disabled={submitting}
                  className="rounded-lg border border-gray-300 px-4 py-2 font-medium transition-colors hover:bg-gray-100 disabled:opacity-50 dark:border-gray-600 dark:hover:bg-gray-700"
                >
                  Cancel Edit
                </button>
              ) : (
          ...
          <div className="mt-8 rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-800">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              <strong>Note:</strong> Each availability block is stored as a separate Arkiv entity.
              You can create multiple blocks for different time periods. When creating offers, you
              can reference these availability blocks.
            </p>
            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
              <strong>Arkiv-native editing:</strong> Editing creates a new entity (original remains
              immutable). Deleting creates a deletion marker entity that filters out the original.
            </p>
          </div>
          ...
          {availabilities.length > 0 && (
            <div className="mt-8">
              <h2 className="mb-4 text-xl font-semibold">All Availability Blocks</h2>
              <div className="space-y-3">
                {availabilities.map((availability) => {
                  // Check if structured format and format accordingly
                  const isStructured = isStructuredAvailability(availability.timeBlocks);
