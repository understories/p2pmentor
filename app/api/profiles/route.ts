...
export async function GET(request: Request) {
  try {
    ...
    const walletsWithMultipleProfiles = Array.from(profileCountsByWallet.entries()).filter(
      ([_, count]) => count > 1
    ).length;
    const walletsWithSingleProfile = Array.from(profileCountsByWallet.entries()).filter(
      ([_, count]) => count === 1
    ).length;
    const totalWallets = profileCountsByWallet.size;
    const percentCanonical =
      totalWallets > 0 ? Math.round((walletsWithSingleProfile / totalWallets) * 100) : 100;
