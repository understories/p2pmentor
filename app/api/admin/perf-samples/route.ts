...
export async function GET(request: Request) {
  ...
  try {
        ...
        if (testArkiv) {
          ...
          try {
            ...
            if (isTimeoutError) {
              ...
              console.log(
                '[seed-perf] Arkiv test - transaction submitted but confirmation pending:',
                {
                  durationMs: duration3,
                  payloadBytes: payloadSize3,
                  txHash,
        ...
        if (testGraphQL) {
          try {
            ...
            try {
              ...
              throw new Error(
                `fetchNetworkOverview failed: ${fetchError?.message || 'Unknown error'}`
              );
            }
            ...
            if (!graphqlData.skillRefs || !Array.isArray(graphqlData.skillRefs)) {
              ...
              throw new Error(
                `GraphQL query returned invalid data structure. Expected skillRefs array, got: ${JSON.stringify(graphqlData).substring(0, 500)}`
              );
            }
            ...
            console.log('[seed-perf] GraphQL query successful:', {
              ...
              totalOffers: graphqlData.skillRefs.reduce(
                (sum, s) => sum + (s.offers?.length || 0),
                0
              ),
            });
            ...
            try {
              ...
              if (isTimeoutError) {
                ...
                console.log(
                  '[seed-perf] GraphQL test - transaction submitted but confirmation pending:',
                  {
                    durationMs: duration4,
                    payloadBytes: payloadSize4,
                    txHash,
            ...
            try {
              ...
              try {
                await fetchProfileDetail(
                  { wallet: CURRENT_WALLET, limitAsks: 1, limitOffers: 1, limitFeedback: 1 },
                  { endpoint: graphqlEndpoint }
                );
                await new Promise((resolve) => setTimeout(resolve, 200)); // Brief pause
              } catch (warmupErr) {
              ...
              let profileData;
              try {
                profileData = await fetchProfileDetail(
                ...
                throw new Error(
                  `fetchProfileDetail failed: ${fetchError?.message || 'Unknown error'}`
                );
              }
              const duration = Date.now() - startTime;
      ...
      if (summaryOperation) {
        ...
        try {
          ...
          if (arkivMetrics.length === 0 && summaryOperation) {
            console.warn(
              '[admin/perf-samples] No Arkiv entities found for operation:',
              summaryOperation
            );
          ...
          if (summaryOperation === 'buildNetworkGraphData') {
            ...
            if (networkOverviewMetrics.length > 0) {
              ...
              const payloadSizes = networkOverviewMetrics
                .filter((m) => m.payloadBytes !== undefined)
                .map((m) => m.payloadBytes!);
              const httpCounts = networkOverviewMetrics
                .filter((m) => m.httpRequests !== undefined)
                .map((m) => m.httpRequests!);
              ...
              if (perfSummary.graphql) {
                ...
                const combinedSamples =
                  (perfSummary.graphql.samples || 0) + networkOverviewMetrics.length;
                const combinedAvg =
                  (perfSummary.graphql.avgDurationMs * (perfSummary.graphql.samples || 0) +
                    (durations.reduce((a, b) => a + b, 0) / durations.length) *
                      networkOverviewMetrics.length) /
                  combinedSamples;

                // Merge page counts
                const mergedPages = {
                  ...(perfSummary.graphql.pages || {}),
                  ...networkOverviewPageCounts,
                };
                Object.keys(mergedPages).forEach((page) => {
                  mergedPages[page] =
                    (perfSummary.graphql?.pages?.[page] || 0) +
                    (networkOverviewPageCounts[page] || 0);
                });

                perfSummary.graphql = {
                  avgDurationMs: combinedAvg,
                  minDurationMs: Math.min(
                    perfSummary.graphql.minDurationMs,
                    Math.min(...durations)
                  ),
                  maxDurationMs: Math.max(
                    perfSummary.graphql.maxDurationMs,
                    Math.max(...durations)
                  ),
                  avgPayloadBytes:
                    payloadSizes.length > 0
                  ...
                  avgHttpRequests:
                    httpCounts.length > 0
                      ? httpCounts.reduce((a, b) => a + b, 0) / httpCounts.length
                      : perfSummary.graphql.avgHttpRequests,
        ...
        try {
          ...
          console.error(
            '[admin/perf-samples] Failed to query Arkiv entities for all operations:',
            error
          );
    ...
    return NextResponse.json({ error: 'Failed to fetch performance samples' }, { status: 500 });
  }
}
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to clear performance samples' }, { status: 500 });
  }
}
