...
export default function AsksPage() {
  ...
  const loadData = async (wallet: string) => {
    try {
      ...
      if (useGraphQL) {
        ...
        try {
          ...
          if (graphqlAsks && Array.isArray(graphqlAsks) && graphqlAsks.length >= 0) {
            ...
            const sortedAsks = mappedAsks.sort(
      ...
      if (asksRes.ok) {
        // Sort by newest first (invert order)
        const sortedAsks = (asksRes.asks || []).sort(
          (a: Ask, b: Ask) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
        );
        setAsks(sortedAsks);
    ...
    try {
      ...
      if (data.ok) {
        ...
        if (data.pending) {
          ...
          setSuccess(
            `Ask created successfully! "${newAsk.skill || 'Your ask'}" is now live and visible to mentors. View it in Network →`
          );
  ...
  const renderAskCard = (ask: Ask) => {
    ...
    const displayName =
      askProfile?.displayName || `${ask.wallet.slice(0, 6)}...${ask.wallet.slice(-4)}`;
    const isMyAsk = walletAddress && ask.wallet.toLowerCase() === walletAddress.toLowerCase();

    return (
      <div key={ask.key} id={ask.key}>
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-6 transition-all duration-300 dark:border-gray-700 dark:bg-gray-800">
          ...
          <p className="mb-3 whitespace-pre-wrap text-gray-700 dark:text-gray-300">{ask.message}</p>
        ...
        {similarAsks.length > 0 && (
          <div className="ml-6 mt-3 border-l-2 border-blue-200 pl-4 dark:border-blue-800">
            ...
            <div className="space-y-2">
              {similarAsks.map((similarAsk) => {
                ...
                const similarDisplayName =
                  ...
                  `${similarAsk.wallet.slice(0, 6)}...${similarAsk.wallet.slice(-4)}`;
                return (
                  <Link
                    key={similarAsk.key}
                    href={`/asks#${similarAsk.key}`}
                    className="block rounded bg-blue-50 p-2 text-sm transition-colors hover:bg-blue-100 dark:bg-blue-900/20 dark:hover:bg-blue-900/30"
                  >
        ...
        <div className="mx-auto max-w-4xl">
              ...
              {arkivBuilderMode ? (
                ...
                >
                  <button
                    ...
                    className={`px-4 py-2 ${askColors.button} flex items-center gap-2 rounded-lg font-medium transition-colors`}
                  >
          ...
          {showCreateForm && (
            <div className="mb-8 rounded-lg border border-gray-200 bg-gray-50 p-6 dark:border-gray-700 dark:bg-gray-800">
              <h2 className="mb-4 text-xl font-semibold">Create New Ask</h2>
              <form onSubmit={handleCreateAsk} className="space-y-4">
                ...
                {(() => {
                  const ttlValue =
                    newAsk.ttlHours === 'custom' ? newAsk.customTtlHours : newAsk.ttlHours;
            ...
            {asks.length === 0 ? (
              ...
              (() => {
                ...
                const myAsks = asks.filter(
                  (ask) => walletAddress && ask.wallet.toLowerCase() === walletAddress.toLowerCase()
                );
                const otherAsks = asks.filter(
                  (ask) =>
                  ...
                  <>
                    ...
                    {myAsks.length > 0 && (
                      <div className="mb-8">
                        ...
                        <div className="space-y-4">{myAsks.map((ask) => renderAskCard(ask))}</div>
                      </div>
                    )}
