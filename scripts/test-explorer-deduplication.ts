...
async function testDeduplication() {
  ...
  console.log(
    `Reduction: ${profiles.length - deduplicatedProfiles.length} profiles (${(((profiles.length - deduplicatedProfiles.length) / profiles.length) * 100).toFixed(1)}% reduction)`
  );

  // Check version counts
  const profilesWithVersions = deduplicatedProfiles.filter(
    (p) => p.versionCount && p.versionCount > 1
  );
  ...
  if (profilesWithVersions.length > 0) {
    ...
    profilesWithVersions.slice(0, 5).forEach((profile) => {
      console.log(
        `  - ${(profile as any).displayName || profile.wallet}: ${profile.versionCount} versions`
      );
    });
  }
  ...
  deduplicatedProfiles.forEach((profile) => {
    if (profile.type === 'profile' && profile.wallet) {
      ...
      if (
        actualVersions > 1 &&
        (!profile.versionCount || profile.versionCount !== actualVersions)
      ) {
        versionCountErrors++;
        console.error(
          `  ❌ Version count mismatch for ${wallet}: expected ${actualVersions}, got ${profile.versionCount}`
        );
