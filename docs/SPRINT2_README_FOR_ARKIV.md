...
2. **Performance Tracking System**
   ...
   - Verifiable on-chain via Braga explorer
...
### 2. Verify on Braga Explorer

Each performance sample is stored as an Arkiv entity with transaction hash:

```
https://explorer.braga.hoodi.arkiv.network/tx/{txHash}
```
...
3. Click 🔗 links to verify on Braga explorer
...
- Every sample has `txHash` for Braga explorer verification
...
- **Verification Links:** Direct links to Braga explorer for on-chain verification
...
**Verifiable on:** Braga Testnet Explorer
