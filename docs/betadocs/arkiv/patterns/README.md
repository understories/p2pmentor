# Development Patterns

Development patterns and best practices for building on Arkiv.

## Patterns

- [Error Handling](/docs/arkiv/patterns/error-handling) - Robust error handling for Arkiv integration
- [Entity Versioning](/docs/arkiv/patterns/entity-versioning) - Pattern A: Creating new entities for versioning scenarios
- [Editable Entities](/docs/arkiv/editable-entities) - Pattern B: Updating entities in place with stable keys ⭐ **Recommended**
- [Designing with Immutable Data](/docs/arkiv/patterns/designing-with-immutable-data) - Core principles for immutable transactions and mutable state
- [Transaction Timeouts](/docs/arkiv/patterns/transaction-timeouts) - Handling transaction timeouts gracefully
- [Query Optimization](/docs/arkiv/patterns/query-optimization) - Optimizing Arkiv queries for performance
- [Deletion Patterns](/docs/arkiv/patterns/deletion-patterns) - Implementing deletion with immutable entities

