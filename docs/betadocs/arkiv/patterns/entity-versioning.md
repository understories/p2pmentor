...
This document describes **Pattern A: Create New Entity per Change** for versioning scenarios. For most mutable application state (profiles, preferences, notifications), use **Pattern B: Update in Place** with stable entity keys instead. See [Editable Entities](/docs/arkiv/overview/editable-entities) for Pattern B.
...
**Note:** This example shows Pattern A (versioning). For mutable application state, use Pattern B (update in place) instead. See [Editable Entities](/docs/arkiv/overview/editable-entities) for Pattern B.
...
```typescript
async function updateProfileWithLock(
  wallet: string,
